      ...
      try {
        ...
        if (response.status && response.data && response.data.notification) {
          ...
          if (response.data.notification.length > 0) {
            ...
            console.log("   Title field: ".concat(sample.title ? '✅' : '❌'));
            console.log("   Description field: ".concat(sample.description || sample.message ? '✅' : '❌'));
            console.log("   Date field: ".concat(sample.date ? '✅' : '❌'));
            console.log('\n✅ All required fields are present for Flutter app');
            console.log('\n💡 Server is ready for Flutter app connections');
          }
        } else {
