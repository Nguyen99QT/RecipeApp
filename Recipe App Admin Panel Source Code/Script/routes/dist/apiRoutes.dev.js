...
var routes = express(); // Importing middleware functions for upload image

var _require = require("../middleware/uploadSingleFile"),
    uploadAvatar = _require.uploadAvatar; // Importing middleware functions for check user authentication


var _require2 = require("../middleware/checkAuthentication"),
    checkAuthorization = _require2.checkAuthorization; // Import controllers


var apiController = require("../controllers/apiController"); //Routes For Sign Up


routes.post("/CheckRegisterUser", apiController.CheckRegisterUser);
routes.post("/Signup", apiController.SignUp);
routes.post("/VerifyOtp", apiController.VerifyOtp); //Routes For Sign In    

routes.post("/SignIn", apiController.SignIn);
routes.post("/isVerifyAccount", apiController.isVerifyAccount);
routes.post("/resendOtp", apiController.resendOtp); //Routes For Forgot Password and Reset Password
...
routes.post("/ResetPassword", apiController.ResetPassword); //Routes For User Edit
...
routes.post("/GetAllCuisines", apiController.GetAllCuisines); //Routes For Recipe
...
routes.post("/SearchRecipes", apiController.SearchRecipes); //Routes For Favourite
...
routes.post("/DeleteFavouriteRecipe", checkAuthorization, apiController.DeleteFavouriteRecipe); //Routes For Reviews
...
routes.post("/GetReviewByRecipeId", apiController.GetReviewByRecipeId); // Admin Routes For Review Management
...
routes.post("/RejectReview", apiController.RejectReview); // Routes For FAQ

routes.post("/getAllFaq", apiController.getAllFaq); // Routes For ads

routes.post("/getAdmob", apiController.getAdmob);
routes.get("/getAdmob", apiController.getAdmob); //Routes For Policy and terms
